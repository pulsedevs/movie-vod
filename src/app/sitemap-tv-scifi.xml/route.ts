...
// src/app/sitemap-tv-scifi.xml/route.ts
...
export async function GET() {
  ...
  if (!apiKey) {
    // Fallback to popular sci-fi TV shows
    const scifiShows = [
      { id: 4607, name: 'Lost' },
    ...
    return generateSciFiTVSitemap(baseUrl, scifiShows)
  }

  try {
    // Fetch sci-fi TV shows from TMDB (Genre ID 10765 = Sci-Fi & Fantasy)
    const controller = new AbortController()
    const timeoutId = setTimeout(() => controller.abort(), 10000)
    
    const allShows = []
    const maxPages = 25 // Fetch 25 pages = ~500 sci-fi shows
    ...
    for (let page = 1; page <= maxPages; page += 3) {
      ...
      for (let i = 0; i < 3 && (page + i) <= maxPages; i++) {
        ...
        batchPromises.push(
          fetch(
            `https://api.themoviedb.org/3/discover/tv?api_key=${apiKey}&language=en-US&page=${currentPage}&with_genres=10765&sort_by=popularity.desc`,
      ...
      try {
        ...
        console.warn(`Sci-fi TV batch error at page ${page}:`, batchError)
        break
      }
    }
    ...
    return generateSciFiTVSitemap(baseUrl, uniqueShows)
    
  } catch (error) {
    console.error('Sci-fi TV sitemap error:', error)
...
function generateSciFiTVSitemap(baseUrl: string, shows: any[]) {
